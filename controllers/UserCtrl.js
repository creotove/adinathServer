...
const createUser = async (req, res) => {
  try {
    ...
    // const exisitingUser = await newUserModel.findOne({ email });
    // if (exisitingUser) {
...
const withdrawCommissionRequest = async (req, res) => {
  try {
    ...
    const reqForWithdrawal = new withdrawalHistory({
      userId,
      amount,
    });
...
const getWithdrawalHistory = async (req, res) => {
  try {
    ...
    const getAllWithdrawal = await withdrawalHistory.find({ userId });
...
const getAccountDetails = async (req, res) => {
  try {
    const { userId } = req.body;
    const accDetails = await AccountDetails.find({ uniqueId: userId });
    // if (!accDetails) {
    //   return res.status(200).send({
